...
def save_preprocessed_data():
    ...
    for series_id in _get_positive_series():
        nodule_coords_annot = annotations[annotations['seriesuid'] == series_id[16:]]
        ...
        ct = CTScan(seriesuid=series_id[16:], centers=tp_co, radii=radii, clazz=1)
        ct.preprocess()
    ...
    for series_id in _get_negative_series():
        nodule_coords_candid = candidates[candidates['seriesuid'] == series_id[16:]]
        ...
        ct = CTScan(seriesuid=series_id[16:], centers=tp_co, radii=radii, clazz=0)
        ct.preprocess()
        ct.save_preprocessed_image()
        diction = ct.get_info_dict()
        meta_data = meta_data.append(pd.Series(diction), ignore_index=True)
